...
void app_main()
{
    ...
    // MAC Address #1 - 10:52:1c:74:76:94 (COM3)
    // MAC Address #2 - 10:52:1c:76:2f:74 (COM6)
    //uint8_t ESPDev2MacAddress[] = {0x10, 0x52, 0x1C, 0x74, 0x76, 0x94};
    uint8_t ESPDev2MacAddress[] = {0x10, 0x52, 0x1C, 0x76, 0x2F, 0x74};
    uint8_t dataD[] = {0x01,0x01,0x01,0x02};
    // espnow_addrt_t espnow_addr = {}; // 

    espnow_add_peer(ESPDev2MacAddress, NULL);
    // esp_err_t returnState = esp_now_send(&ESPDev2MacAddress); // send to this specific one
    
    // send to everyone
    esp_err_t returnState = esp_now_send(NULL, dataD, 4);
    
    // send
    if (returnState == ESP_OK) {
        printf("Sending has succeded!\n");
    }
    else {
        printf("Failed return code: %d", returnState);
    }
